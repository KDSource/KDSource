...
class KDSource:
    ...
    def plot_t(self, grid_t, vec0=None, vec1=None, **kwargs):
        ...
        **kwargs: optional
        ...
        if np.sum(mask) == 0:
            raise Exception("No particles in [vec0,vec1] range.")
        vecs = self.kde.data[:, -1][mask].reshape(-1, 1)  #### this will be true as long as time is the last defined metric
        ws = self.kde.weights[mask]
        N_eff = np.sum(ws) ** 2 / np.sum(ws ** 2)
        scaling = self.scaling[-1] #### this will be true as long as time is the last defined metric
        bw = self.kde.bw
        if kwargs["adjust_bw"]:
            bw *= bw_silv(1, N_eff) / bw_silv(self.geom.dim, self.N_eff)
        kde = TreeKDE(bw=bw)
        kde.fit(vecs, weights=ws)
        grid = self.geom.ms[-1].transform(grid_t) #### this will be true as long as time is the last defined metric
        jacs = self.geom.ms[-1].jac(grid_t) #### this will be true as long as time is the last defined metric
        scores = 1 / scaling * kde.evaluate(grid.reshape(-1, 1) / scaling)
        errs = np.sqrt(scores * R_gaussian / (N_eff * np.mean(bw) * scaling))
        scores *= self.J * np.sum(ws) / np.sum(self.kde.weights) * jacs
        errs *= self.J * np.sum(ws) / np.sum(self.kde.weights) * jacs
