...
def read_bashoutput(bashoutput, mccode):
    ...
    Searches for number of produced particles (with KDSource) and
    simulation times.
...
class Summary:
    ...
    def compute(self):
        ...
        if self.bashoutput is not None:
            self.t_simul, self.I_source = read_bashoutput(
                self.folder + "/" + self.bashoutput, self.mccode
            )
        else:
            self.t_simul = 0
            self.I_source = 1
        ...
        for detector in self.n_detectors:
            if self.mccode == "McStas":
                readformat = "mcpl"
            if self.mccode == "TRIPOLI":
                readformat = "stock"
        ...
        if self.t4output is not None:
            for tallyname in self.tallies:
                ...
                self.tally_scores.append(
                    [
                        self.I_source * np.sum(tally.I) * tally.vcell,
                        self.I_source
                        * np.sqrt(np.sum(tally.err ** 2))
                        * tally.vcell,
                    ]
                )
