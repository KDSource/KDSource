...
int Geom_perturb(const Geometry *geom, mcpl_particle_t *part) {
  ...
  if (geom->trasl)
    traslv(part->position, geom->trasl, 0);
  return ret;
}
...
int Geom_next(Geometry *geom, int loop) {
  if (geom->bwfile) {
    ...
    if (!loop)
      KDS_end("Geom_next: End of BW file reached.");
...
int Vol_perturb(const Metric *metric, mcpl_particle_t *part, double bw,
  ...
  while ((part->position[2] < zmin) || (part->position[2] > zmax)) {
    if (part->position[2] < zmin)
      ...
      part->position[2] -= 2 * (part->position[2] - zmax);
  }
  return 0;
}
...
int SurfR_perturb(const Metric *metric, mcpl_particle_t *part, double bw,
  ...
  rho = sqrt(part->position[0] * part->position[0] +
...
int SurfR2_perturb(const Metric *metric, mcpl_particle_t *part, double bw,
  ...
  rho = part->position[0] * part->position[0] +
        part->position[1] * part->position[1];
  psi = atan2(part->position[1], part->position[0]);
...
int Guide_perturb(const Metric *metric, mcpl_particle_t *part, double bw,
  ...
  if (rcurv != 0) { // Transform to curved guide variables
    ...
    dx2 = dx;
    dz2 = dz;
    dx = dx2 * cos(z / rcurv) + dz2 * sin(z / rcurv);
    dz = -dx2 * sin(z / rcurv) + dz2 * cos(z / rcurv);
  }
  // Transform from (x,y,z,dx,dy,dz) to (z,t,mu,phi)
  if ((y / yheight > -x / xwidth) && (y / yheight < x / xwidth))
    mirror = 0; // mirror x pos
  else if ((y / yheight > x / xwidth) && (y / yheight > -x / xwidth))
    mirror = 1; // mirror y pos
  else if ((y / yheight < -x / xwidth) && (y / yheight > x / xwidth))
    mirror = 2; // mirror x neg
  else
    mirror = 3; // mirror y neg
  switch (mirror) {
  case 0:
    t = 0.5 * yheight + y;
    mu = dx;
  ...
  case 1:
    while ((t < yheight) || (t > yheight + xwidth)) {
      if (t < yheight)
        ...
        t -= 2 * (t - (yheight + xwidth));
    }
    break;
  case 2:
    while ((t < yheight + xwidth) || (t > 2 * yheight + xwidth)) {
      if (t < yheight + xwidth)
        t += 2 * (yheight + xwidth - t);
      else
        t -= 2 * (t - (2 * yheight + xwidth));
    }
    break;
  case 3:
    while ((t < 2 * yheight + xwidth) || (t > 2 * yheight + 2 * xwidth)) {
      if (t < 2 * yheight + xwidth)
        t += 2 * (2 * yheight + xwidth - t);
      else
        t -= 2 * (t - (2 * yheight + 2 * xwidth));
    }
    break;
  }
  ...
  if (isinf(metric->scaling[3]))
    ...
    phi += bw * metric->scaling[3] * M_PI / 180 * rand_type(kernel);
  ...
  case 0:
    ...
    y = t - 0.5 * yheight;
    dx = mu;
    dz = sqrt(1 - mu * mu) * cos(phi);
    dy = -sqrt(1 - mu * mu) * sin(phi);
    break;
  case 1:
    y = yheight / 2;
    x = -t + yheight + 0.5 * xwidth;
    dy = mu;
    dz = sqrt(1 - mu * mu) * cos(phi);
    dx = sqrt(1 - mu * mu) * sin(phi);
    break;
  case 2:
    x = -xwidth / 2;
    y = -t + 1.5 * yheight + xwidth;
    dx = -mu;
    dz = sqrt(1 - mu * mu) * cos(phi);
    dy = sqrt(1 - mu * mu) * sin(phi);
    break;
  ...
  if (rcurv != 0) { // Antitransform curved guide variables
    ...
    dx2 = dx;
    dz2 = dz;
    dx = dx2 * cos(ang) - dz2 * sin(ang);
    dz = dx2 * sin(ang) + dz2 * cos(ang);
  }
  part->position[0] = x;
  part->position[1] = y;
  part->position[2] = z;
...
int Isotrop_perturb(const Metric *metric, mcpl_particle_t *part, double bw,
  ...
  if (isinf(bw * metric->scaling[0])) { // Generate isotropic direction
    ...
    if (keepx && (dx * part->direction[0] < 0))
      part->direction[0] *= -1;
    if (keepy && (dy * part->direction[1] < 0))
      part->direction[1] *= -1;
    if (keepz && (dz * part->direction[2] < 0))
      part->direction[2] *= -1;
  }
  return 0;
}
