...
void display_usage(){
	printf("Usage: kstool beamtest sourcefile [options]\n\n");
...
int beamtest_parse_args(int argc, char **argv, const char** sourcefile,
		long* N, const char** results, double* xwidth, double* yheight, double* z, double* xshift, double* yshift){
	*sourcefile = 0;
	*N = 1E6;
...
int propagate_collimator(mcpl_particle_t* part, double xwidth, double yheight, double z, double xshift, double yshift){
	if((part->position[2] - z) * part->direction[2] > 0)
		return 0;
	if(part->ekin <= 0)
		return 0;
	double t = (z-part->position[2]) / part->direction[2]; // Time to reach collimator assuming v=1
...
int main(int argc, char *argv[]){
	...
	long int N;
	if(beamtest_parse_args(argc, argv, &sourcefile, &N, &results, &xwidth, &yheight, &z, &xshift, &yshift)) return 1;

	// KSource object
	KSource* ks = KS_open(sourcefile);
	mcpl_particle_t part;
	double w_crit = KS_w_mean(ks, 1000, NULL);

	// Variables to store results
	long int N_source_KDE=0, N_source_tracks=0;
	double I_source_KDE=0, I_source_tracks=0, I_KDE_tot=0, I_tracks_tot=0, err_KDE_tot=0, err_tracks_tot=0;
	int ngroups = 4;
	double Ecuts[] = {0, 1E-8, 3E-7, 0.5, 100}; // Energy groups cuts
	double I_KDE[] = {0,0,0,0}, I_tracks[] = {0,0,0,0}, err_KDE[] = {0,0,0,0}, err_tracks[] = {0,0,0,0};
	long int i;
	int j;

	// KDE simulation
	mcpl_rewind(ks->plist->file);
	if(ks->geom->bwfile) rewind(ks->geom->bwfile);
	...
	for(i=0; i<N; i++){
		if(KS_sample2(ks, &part, 0, w_crit, NULL, 1)) break;
		N_source_tracks++;
	...
	// Normalize results
	for(j=0; j<ngroups; j++){
		I_KDE[j] /= I_source_KDE; I_tracks[j] /= I_source_tracks;
		I_KDE_tot += I_KDE[j]; I_tracks_tot += I_tracks[j];
		err_KDE_tot += err_KDE[j]; err_tracks_tot += err_tracks[j];
		err_KDE[j] = sqrt(err_KDE[j])/I_source_KDE; err_tracks[j] = sqrt(err_tracks[j])/I_source_tracks;
	}
	err_KDE_tot = sqrt(err_KDE_tot)/I_source_KDE; err_tracks_tot = sqrt(err_tracks_tot)/I_source_tracks;

	// Save results
	FILE* resfile = fopen(results, "w");
	fprintf(resfile, "Results are given for source intensity = 1.\n");
	fprintf(resfile, "Energy group cuts: [ ");
	for(j=0; j<ngroups; j++) fprintf(resfile, "%.3le, ", Ecuts[j]);
	fprintf(resfile, "%.3le ]\n\n", Ecuts[ngroups]);
	fprintf(resfile, "E group\tJ KDE\terr\tJ tracks\terr\tDiff.\terr\tRel. diff. [%%]\terr\n");
	...
	for(j=0; j<ngroups; j++){
		...
		err_rd = reldif * sqrt((err_dif/I_dif)*(err_dif/I_dif) + (err_tracks[j]/I_tracks[j])*(err_tracks[j]/I_tracks[j]));
		fprintf(resfile, "%d\t%.3le\t%.1le\t%.3le\t%.1le\t%.3le\t%.1le\t%.2lf\t%.1lf\n", j+1,
			I_KDE[j], err_KDE[j], I_tracks[j], err_tracks[j],
	...
	err_rd = fabs(reldif * sqrt((err_dif/I_dif)*(err_dif/I_dif) + (err_tracks_tot/I_tracks_tot)*(err_tracks_tot/I_tracks_tot)));
